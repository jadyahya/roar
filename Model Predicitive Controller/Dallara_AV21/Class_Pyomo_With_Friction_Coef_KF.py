...
class Dallara_WMPC:
    def __init__(self, xd, yd, speed, sideslipangle, wheel_speed, ca, cs, mux, muy, reff, tf, tr, lf, lr, ms, hs,
                 m, ixx, ksf, ksr, ls, g, bsf,bsr, iz, kus_d, rmax, Ts, Q, R, W0, phir, alphamax, Qmax, deltamax, x0, N, P_t1, experim_param,
                 lamb, kr_1, mux_1, a):
        self.xd = xd
        self.yd = yd
        self.speed = speed
        ...
        #For EKF
        self.P_t1 = P_t1
        self. experim_param = experim_param
        self.lamb = lamb
        self.mux_1 = mux_1
        self.kr_1 = kr_1
        self.a = a #slope of relation between slip slope and mux
        return

    def mux_estimator(self, slip_ratio, fz0, fx0,theta_t1):
        yt = fx0[0]+fx0[1]+fx0[2]+fx0[3]
        if slip_ratio[2]<0.1:
            phi_t = self.experim_param[0]*fz0[0]*slip_ratio[0] + self.experim_param[1]*fz0[1]*slip_ratio[1]+fz0[2]*slip_ratio[2]+fz0[3]*slip_ratio[3]
            error = yt - phi_t*self.kr_1
            K_t = self.P_t1 * phi_t / (self.lamb + phi_t * self.P_t1 * phi_t)
            P_t = 1 / self.lamb * (self.P_t1 - (self.P_t1 * phi_t * phi_t * self.P_t1) / (self.lamb + phi_t * self.P_t1 * phi_t))
            theta_t = theta_t1 + K_t * error
            mux = self.a * theta_t # a to be defined later by experimentation
            return mux  # Here this is the slip slope

        else:
            phi_t = fz0[0]+fz0[1]+fz0[2]+fz0[3]
            error = yt - phi_t * self.mux_1
            K_t = self.P_t1 * phi_t / (self.lamb + phi_t * self.P_t1 * phi_t)
            P_t = 1 / self.lamb * (self.P_t1 - (self.P_t1 * phi_t * phi_t * self.P_t1) / (self.lamb + phi_t * self.P_t1 * phi_t))
            theta_t = theta_t1 + K_t * error
            return mux  #Here this is the coefficient of friction

    def longloadestim(self,slip_ratio):
        fx0 = np.zeros((1,4))
        for i in range(4):
            fx0[i] = self.cs[i]*slip_ratio
        return fx0
    ...
    def ctrl_action(self):
        ...
        u,v = self.speed_estimation(self)
        ...
        fz0 = self.load_estimator(self)
        slip_angle = self.slip_angle_estimator(self, u)
        slip_ratio = self.slip_ratio_estimator(self,u)
        ...
        model.tidx = pyo.Set(initialize=range(0, self.N + 1))
        model.tidu = pyo.Set(initialize=range(0, self.N))
